...
class ControllerMultisellerPayment extends ControllerMultisellerBase {
	public function getTableData() {
		...
		foreach ($results as $result) {
			...
			if (!empty($result['payment_data']) && filter_var($result['payment_data'], FILTER_VALIDATE_EMAIL)) { 
				$actions .= "<a class='btn btn-info' title='" . $this->language->get('ms_payment_payout_paypal') . "'><i class='fa fa-paypal'></i></a> &nbsp;";
			} else {
				$actions .= "<a class='btn btn-warning' title='" . $this->language->get('ms_payment_payout_paypal_invalid') . "'><i class='fa fa-paypal'></i></a>&nbsp;";
			}
			}
			if ($result['amount'] > 0 && $result['payment_status'] == MsPayment::STATUS_UNPAID) { 
				$actions .= "<a class='btn btn-success' title='" . $this->language->get('ms_payment_mark') . "'><i class='fa fa-check'></i></a>&nbsp;";
			}
			$actions .= "<a class='btn btn-danger' title='" . $this->language->get('ms_payment_delete') . "'><i class='fa fa-trash'></i></a>";
	...
	public function create() {
		...
		foreach ($msPayment->getConstants() as $cname => $cval) {
                        if($cname !="TYPE_SIGNUP" && $cname !="TYPE_LISTING") {
