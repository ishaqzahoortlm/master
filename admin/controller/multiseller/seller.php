...
class ControllerMultisellerSeller extends ControllerMultisellerBase {
		...
		foreach ($results as $result) {
			...
			if ($this->currency->format($this->MsLoader->MsBalance->getSellerBalance($result['seller_id']) - $this->MsLoader->MsBalance->getReservedSellerFunds($result['seller_id']), $this->config->get('config_currency'), '', FALSE) > 0) {
				if (!empty($result['ms.paypal']) && filter_var($result['ms.paypal'], FILTER_VALIDATE_EMAIL)) {
					$actions .= "<a class='btn btn-info' title='" . $this->language->get('ms_catalog_sellers_balance_paypal') . "'><i class='fa fa-paypal'></i></a> &nbsp";
				} else {
					$actions .= "<a class='btn btn-warning' title='".$this->language->get('ms_payment_payout_paypal_invalid') . "'><i class='fa fa-paypal'></i></a> &nbsp";
				}
			}
			$actions .= "<a class='btn btn-primary' href='" . $this->url->link('multiseller/seller/update', 'token=' . $this->session->data['token'] . '&seller_id=' . $result['seller_id'], 'SSL') . "' title='".$this->language->get('button_edit')."'><i class='fa fa-pencil'></i></a> &nbsp";
			$actions .= "<a class='btn btn-danger' href='" . $this->url->link('multiseller/seller/delete', 'token=' . $this->session->data['token'] . '&seller_id=' . $result['seller_id'], 'SSL') . "' title='".$this->language->get('button_delete')."'><i class='fa fa-trash'></i></a>";
			...
			$columns[] = array_merge(
				...
				array(
					...
					'seller' => "<a href='".$this->url->link('sale/customer/edit', 'token=' . $this->session->data['token'] . '&customer_id=' . $result['seller_id'], 'SSL')."'>{$result['c.name']} ({$result['ms.company']})</a>",
					...
					'status' => $this->language->get('ms_seller_status_' . $result['ms.seller_status']),
                                        'featured' => ($result['ms.featured']==1)?'Yes':'No',        
	...
	public function create() {
		...
		$this->data['seller'] = FALSE;
                ...
                $this->data['all_package'] = $all_package;
