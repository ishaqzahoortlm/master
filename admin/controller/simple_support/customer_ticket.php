	...
	class ControllerSimpleSupportCustomerTicket extends Controller {
		...
		public function insert() {
			...
			if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
				...
				$this->response->redirect($this->url->link('simple_support/customer_ticket', 'token=' . $this->session->data['token'] . $url, 'SSL'));
			}
		...
		public function getList() {
			...
			if($this->config->get('simple_support_ticket_permission')) {
				...
				if(in_array($user_group_info['user_group_id'], $simple_support_ticket_permission)) {
					...
					$data['view_all_ticket'] = $this->url->link('simple_support/customer_ticket', 'token=' . $this->session->data['token'] . '&view_all=1', 'SSL');
				}	
			}
			
			$data['insert'] = $this->url->link('simple_support/customer_ticket/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
			$data['delete'] = $this->url->link('simple_support/customer_ticket/delete', 'token=' . $this->session->data['token'] . $url, 'SSL');	
			...
			$ticket_total = $this->model_simple_support_customer_ticket->getTotalTickets($filter_data);
			
			$results = $this->model_simple_support_customer_ticket->getTickets($filter_data);
			...
			foreach ($results as $result) {
				...
				$data['tickets'][] = array(
					...
					'edit'          				=> $this->url->link('simple_support/customer_ticket/update', 'token=' . $this->session->data['token'] . '&simple_support_ticket_id=' . $result['simple_support_ticket_id'] . $url, 'SSL')
				);
			}	
			...
			$data['sort_ticket'] = $this->url->link('simple_support/customer_ticket', 'token=' . $this->session->data['token'] . '&sort=sst.ticket_id' . $url, 'SSL');
			$data['sort_customer'] = $this->url->link('simple_support/customer_ticket', 'token=' . $this->session->data['token'] . '&sort=customer' . $url, 'SSL');
			$data['sort_subject'] = $this->url->link('simple_support/customer_ticket', 'token=' . $this->session->data['token'] . '&sort=sst.subject' . $url, 'SSL');
			$data['sort_ticket_status'] = $this->url->link('simple_support/customer_ticket', 'token=' . $this->session->data['token'] . '&sort=sst.simple_support_ticket_status_id' . $url, 'SSL');
			$data['sort_status'] = $this->url->link('simple_support/customer_ticket', 'token=' . $this->session->data['token'] . '&sort=sst.status' . $url, 'SSL');
			$data['sort_date_added'] = $this->url->link('simple_support/customer_ticket', 'token=' . $this->session->data['token'] . '&sort=sst.date_added' . $url, 'SSL');
			$data['sort_date_modified'] = $this->url->link('simple_support/customer_ticket', 'token=' . $this->session->data['token'] . '&sort=sst.date_modified' . $url, 'SSL');
            ...
            $this->response->setOutput($this->load->view('simple_support/customer_ticket_list.tpl', $data));	
		}
		...
		public function getForm() {
			...
			$data['departments'] = $this->model_simple_support_customer_ticket->getDepartments();
			
			if (!isset($this->request->get['simple_support_ticket_id'])) {
				$data['action'] = $this->url->link('simple_support/customer_ticket/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
			} else {
				$data['action'] = $this->url->link('simple_support/customer_ticket/update', 'token=' . $this->session->data['token'] . '&simple_support_ticket_id=' . $this->request->get['simple_support_ticket_id'] . $url, 'SSL');
			}
			
			$data['cancel'] = $this->url->link('simple_support/customer_ticket', 'token=' . $this->session->data['token'] . $url, 'SSL');
            ...
            $this->response->setOutput($this->load->view('simple_support/customer_ticket_form.tpl', $data));			
		}	
		...
		public function upload() {
    		$this->load->language('simple_support/customer_ticket');
