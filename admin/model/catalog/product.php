...
class ModelCatalogProduct extends Model {
    ...
    public function addProduct($data) {
        $certification_id = '';
        if ($data['certifications'] == 0 && $data['certification_other'] != "") {
            $this->db->query("INSERT INTO " . DB_PREFIX . "certification SET status = '1'");
            $certification_id = $this->db->getLastId();
            $this->db->query("INSERT INTO " . DB_PREFIX . "certification_description SET certification_id = '" . (int) $certification_id . "', language_id = '1', title = '" . $this->db->escape($data['certification_other']) . "'");
        } else {
            $certification_id = $data['certifications'];
        }

        $this->event->trigger('pre.admin.product.add', $data);

        $this->db->query("INSERT INTO " . DB_PREFIX . "product SET certifications = '" . $certification_id . "', model = '" . $this->db->escape($data['model']) . "', sku = '" . $this->db->escape($data['sku']) . "', upc = '" . $this->db->escape($data['upc']) . "', ean = '" . $this->db->escape($data['ean']) . "', jan = '" . $this->db->escape($data['jan']) . "', isbn = '" . $this->db->escape($data['isbn']) . "', mpn = '" . $this->db->escape($data['mpn']) . "', location = '" . $this->db->escape($data['location']) . "', quantity = '" . (int) $data['quantity'] . "', minimum = '" . (int) $data['minimum'] . "', subtract = '" . (int) $data['subtract'] . "', stock_status_id = '" . (int) $data['stock_status_id'] . "', date_available = '" . $this->db->escape($data['date_available']) . "', manufacturer_id = '" . (int) $data['manufacturer_id'] . "', shipping = '" . (int) $data['shipping'] . "', price = '" . (float) $data['price'] . "', points = '" . (int) $data['points'] . "', weight = '" . (float) $data['weight'] . "', weight_class_id = '" . (int) $data['weight_class_id'] . "', length = '" . (float) $data['length'] . "', width = '" . (float) $data['width'] . "', height = '" . (float) $data['height'] . "', length_class_id = '" . (int) $data['length_class_id'] . "', status = '" . (int) $data['status'] . "', tax_class_id = '" . $this->db->escape($data['tax_class_id']) . "', sort_order = '" . (int) $data['sort_order'] . "', date_added = NOW()");
    ...
    public function editProduct($product_id, $data) {
        ...
        $this->db->query("UPDATE " . DB_PREFIX . "ms_product SET product_status='".$data['status']."' , product_approved='".$data['status']."'  WHERE product_id = '" . (int) $product_id . "' "  );
