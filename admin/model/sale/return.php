...
class ModelSaleReturn extends Model {
		...
		if ($data['notify']) {
			...
			if ($return_query->num_rows) {
				...
				*/
                                ...
                                if ($data['comment']) {
                                    $message .='<p>'.$this->language->get('text_comment').' '.strip_tags(html_entity_decode($data['comment'], ENT_QUOTES, 'UTF-8')).'</p>';
                                }
                                $message .='</td>
                                </tr>';
                        ...
                        $html = $this->load->view('mail/template.tpl', $data);
	...
	public function getTotalReturnHistoriesByReturnStatusId($return_status_id) {
        ...
        public function getSellerInfo($product_id){
            $sq = "select s.company from  " . DB_PREFIX . "ms_seller as s, " . DB_PREFIX . "ms_product as p where s.seller_id = p.seller_id AND p.product_id = '".$product_id."' ";
            $query = $this->db->query($sq);
            return isset($query->row['company'])?$query->row['company']:'';
