...
class ControllerSellerAccountProduct extends ControllerSellerAccount {
        ...
        foreach ($products as $product) {
            ...
            if ($product['mp.product_status'] != MsProduct::STATUS_DISABLED) {
                if ($product['mp.product_status'] == MsProduct::STATUS_ACTIVE)
                    $actions .= "<a href='" . $this->url->link('product/product', 'product_id=' . $product['product_id'], 'SSL') . "' class='btn-sm btn-default' title='" . $this->language->get('ms_viewinstore') . "'><i class='fa fa-search'></i></a>&nbsp";

                if ($product['mp.product_approved']) {
                    if ($product['mp.product_status'] == MsProduct::STATUS_INACTIVE)
                        $actions .= "<a href='" . $this->url->link('seller/account-product/publish', 'product_id=' . $product['product_id'], 'SSL') . "' class='btn-sm btn-success' title='" . $this->language->get('ms_publish') . "'><i class='fa fa-check'></i></a>&nbsp";

                    if ($product['mp.product_status'] == MsProduct::STATUS_ACTIVE)
                        $actions .= "<a href='" . $this->url->link('seller/account-product/unpublish', 'product_id=' . $product['product_id'], 'SSL') . "' class='btn-sm btn-warning' title='" . $this->language->get('ms_unpublish') . "'><i class='fa fa-ban'></i></a>&nbsp";
                }

                $actions .= "<a href='" . $this->url->link('seller/account-product/update', 'product_id=' . $product['product_id'], 'SSL') . "' class=' btn-sm btn-primary custom_pading_btn ' title='" . $this->language->get('ms_edit') . "'><i class='fa fa-pencil'></i></a>&nbsp";
                $actions .= "<a href='" . $this->url->link('seller/account-product/update', 'product_id=' . $product['product_id'] . "&clone=1", 'SSL') . "' class='btn-sm btn-default' title='" . $this->language->get('ms_clone') . "'><i class='fa fa-file-o'></i></a>&nbsp";
                $actions .= "<a href='" . $this->url->link('seller/account-product/delete', 'product_id=' . $product['product_id'], 'SSL') . "' class='btn-sm btn-danger button-delete' title='" . $this->language->get('ms_delete') . "'><i class='fa fa-trash-o'></i></a>&nbsp";
            } else {
                if ($this->config->get('msconf_allow_relisting')) {
                    $actions .= "<a href='" . $this->url->link('seller/account-product/update', 'product_id=' . $product['product_id'] . "&relist=1", 'SSL') . "' class='ms-button ms-button-relist' title='" . $this->language->get('ms_relist') . "'></a>&nbsp";
                }
            }
    ...
    private function _initForm() {
        ...
        $rs_package_detail = $this->model_extension_package->getPackage($this->data['seller']['ms.package_id']);
        
        if($rs_package_detail['modules']!="") {
          $package_modules_array = explode(',',$rs_package_detail['modules']);
        }else {
          $package_modules_array = array();  
        }
        $this->data['package_modules'] = $package_modules_array;
        ...
        $this->data['ms_list_certification'] = $this->language->get('ms_list_certification');

        // Certifications
        $this->load->model('extension/certification');
        $res_all_cert = $this->model_extension_certification->getAll();
        $this->data['all_certification'] = $res_all_cert;
    ...
    public function update() {
        ...
        if ($this->MsLoader->MsProduct->productOwnedBySeller($product_id, $seller_id)) {
            $product = $this->MsLoader->MsProduct->getProduct($product_id);
            $this->data['certifications']= $product['certifications'];
        } else {
            $product = NULL;
        }
