	...
	public function importItems($data) {
		...
		foreach ($new_data_1 as $item) {
			if (!in_array($item['ItemID'], $current)) {
				...
				$length_class_id = 1;
				if (isset($item['advanced']['certification']['size']['width_unit']) && !empty($item['advanced']['certification']['size']['width_unit'])) {
					$length_class_id = $this->lengthClassExists($item['advanced']['certification']['size']['width_unit']);
				}

				//get the weight class id
				$weight_class_id = 1;
				if (isset($item['advanced']['certification']['weight']['major_unit']) && !empty($item['advanced']['certification']['weight']['major_unit'])) {
					$weight_class_id = $this->weightClassExists($item['advanced']['certification']['weight']['major_unit']);
				}
				...
				if (isset($item['advanced']['certification']['weight']['major'])) {
					$weight = $item['advanced']['certification']['weight']['major'] . ' . ' . $item['advanced']['certification']['weight']['minor'];
				} else {
					$weight = 0;
				}

				//certification length
				if (isset($item['advanced']['certification']['size']['length'])) {
					$length = $item['advanced']['certification']['size']['length'];;
				} else {
					$length = 0;
				}

				//certification width
				if (isset($item['advanced']['certification']['size']['width'])) {
					$width = $item['advanced']['certification']['size']['width'];;
				} else {
					$width = 0;
				}

				//certification height
				if (isset($item['advanced']['certification']['size']['height'])) {
					$height = $item['advanced']['certification']['size']['height'];;
				} else {
					$height = 0;
				}
