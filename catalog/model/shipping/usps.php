				...
				if ($result) {
					...
					if ($rate_response || $intl_rate_response) {
						if ($address['iso_code_2'] == 'US') {
							...
							$certification = $rate_response->getElementsByTagName('certification')->item(0);

							$postages = $certification->getElementsByTagName('Postage');
							...
							if ($postages->length) {
								...
								$error = $certification->getElementsByTagName('Error')->item(0);
							...
							$certification = $intl_rate_response->getElementsByTagName('certification')->item(0);

							$services = $certification->getElementsByTagName('Service');
