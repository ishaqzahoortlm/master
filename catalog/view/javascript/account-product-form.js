$(function() {
	...
	$('body').delegate("a.ms-button-delete", "click", function() {
        ...
        $('body').delegate("a.custom-button-delete", "click", function() {
		$(this).parents('tr').remove();
	});
			...
			FileUploaded: function(up, file, info) {
				...
				if (!$.isEmptyObject(data.errors)) {
					...
					$("."+up.id+".error").append(errorText).fadeIn(1000);
                                ...
                                var current_images = $(".product_image_files img").length;
                                if(maximum_images==0){
				if (!$.isEmptyObject(data.files)) {
					for (var i = 0; i < data.files.length; i++) {
						...
						'</div>').children(':last').hide().fadeIn(2000);
                                ...
                                }else if(up.files.length >=maximum_images && current_images==0) { 
                                    if (!$.isEmptyObject(data.files)) {
					for (var i = 0; i < data.files.length; i++) {
						$(".product_image_files").append(
						'<div class="ms-image">' +
						...
						'</div>').children(':last').hide().fadeIn(2000);
                                ...
                                else if(up.files.length <=maximum_images ) { 
                                    if (!$.isEmptyObject(data.files)) {
					for (var i = 0; i < data.files.length; i++) {
						$(".product_image_files").append(
						'<div class="ms-image">' +
