...
final class MsSeller extends Model {
	...
	public function editSeller($data) {
        ...
        public function editSeller2($data) {
		$seller_id = (int)$data['seller_id'];
		...
		$sql = "UPDATE " . DB_PREFIX . "ms_seller
				SET description = '" . $this->db->escape($data['mission']) . "',
					company = '" . $this->db->escape($data['company']) . "',
					nickname = '" . $this->db->escape($data['nickname']) . "',
					country_id = " . (int)$data['country'] . ",
					zone_id = " . (int)$data['zone'] . ","
					...
					. (isset($data['approved']) ? "seller_approved=  " .  (int)$data['approved'] . "," : '')
					. "paypal = '" . $this->db->escape($data['paypal']) . "',
					billing_address = '" . $this->db->escape($data['billing_address']) . "',
                                        city = '" . $this->db->escape($data['city']). "', 
                                        street = '" . $this->db->escape($data['street']) . "', 
                                        website = '" . $this->db->escape($data['website']) . "',
                                        category_id = '" . implode(',',$data['category_id']) . "',
                                        organization_type = '" . $this->db->escape($data['organization_type']) . "',
                                        revenue = '" . $this->db->escape($data['revenue']) . "',
                                        leader_ship = '" . json_encode($data['leader_ship']) . "',
                                        authorized_contact = '" . json_encode($data['authorized_contact']) . "',    
                                        countries = '" . implode(',',$data['countries']) . "',  
                                        shipping_type = '" . $this->db->escape($data['shipping_type']) . "',
                                        dunn_number = '" . $this->db->escape($data['dunn_number']) . "',
                                        eid = '" . $this->db->escape($data['eid']) . "',
                                        moq = '" . $this->db->escape($data['moq']) . "',
                                        factory_lead_time = '" . $this->db->escape($data['factory_lead_time']) . "',
                                        warranty_option = '" . $this->db->escape($data['warranty_option']) . "',
                                        warranty_des = '" . $this->db->escape($data['warranty_des']) . "',    
					membership_type = '" . $this->db->escape($data['membership_type']) . "'
				WHERE seller_id = " . (int)$seller_id;
	...
	public function getTotalSellers($data = array()) {
        ...
        public function getTotalSellersFeatured($data = array()) {
		$sql = "
			SELECT COUNT(*) as total
			FROM " . DB_PREFIX . "ms_seller ms
			WHERE featured = '1' "
	...
	public function getSeller($seller_id, $data = array()) {
		$sql = "SELECT	CONCAT(c.firstname, ' ', c.lastname) as name,
						c.email as 'c.email',
                                                c.newsletter as 'c.newsletter',
						ms.seller_id as 'seller_id',
						ms.nickname as 'ms.nickname',
                                                ms.billing_address as 'ms.billing_address',
                                                ms.city as 'ms.city',
                                                ms.street as 'ms.street',
                                                ms.category_id as 'ms.category_id',
                                                ms.package_id as 'ms.package_id',
						ms.company as 'ms.company',
                                                ms.featured as 'ms.featured',
						ms.website as 'ms.website',
                                                ms.p_contact_f_name as 'ms.p_contact_f_name',
                                                ms.p_contact_l_name as 'ms.p_contact_l_name',
                                                ms.p_contact_phone as 'ms.p_contact_phone',
                                                ms.p_contact_email as 'ms.p_contact_email',
                                                ms.moq as 'ms.moq',
                                                ms.warranty_option as 'ms.warranty_option',
                                                ms.warranty_des as 'ms.warranty_des',
                                                ms.factory_lead_time as 'ms.factory_lead_time',
                                                ms.eid as 'ms.eid',
                                                ms.dunn_number as 'ms.dunn_number',
                                                ms.countries as 'ms.countries',
                                                ms.shipping_type as 'ms.shipping_type',
                                                ms.membership_type as 'ms.membership_type',
                                                ms.organization_type as 'ms.organization_type',
                                                ms.revenue as 'ms.revenue',
                                                ms.leader_ship as 'ms.leader_ship',
                                                ms.authorized_contact as 'ms.authorized_contact',
	...
	public function getSellers($data = array(), $sort = array(), $cols = array()) {
		...
		$sql = "SELECT
				...
				. (isset($data['seller_status']) ? " AND seller_status IN  (" .  $this->db->escape(implode(',', $data['seller_status'])) . ")" : '')
				. (isset($data['featured']) ? " AND featured = 1 " : '')
